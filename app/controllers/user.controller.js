...
exports.get_home = (req, res)=>{
    ...
    User.findOne({ '_id': user._id }).populate({ path: 'posts', populate: { path: 'group' } }).exec(async (err, user)=>{
        if (err)
            res.send(err)
        if (user == null){
            res.writeHead(302, {
        ...
        User.find({'_id':{$in:user.friends}}).populate({path:'posts',populate: { path: 'group' }}).exec(async(err, users)=>{
            ...
            await Group.find({}).exec(async (err, groups)=>{
                console.log(home_users[0].posts[5])
...
exports.add_friend = (req, res)=>{
    ...
    User.findByIdAndUpdate({_id:user_id}, {$addToSet:{'friends':req.query._id}}).exec(async (err, user)=>{
        if (err)
            res.send(err);
        localStorage.setItem('user', JSON.stringify(user));
        await User.findByIdAndUpdate({_id:req.query._id}, {$addToSet:{'friends':user_id}}).exec(async(err, user)=>{
            if (err)
                res.send(err);
                console.log(user);
           //localStorage.setItem('user', JSON.stringify(user));
